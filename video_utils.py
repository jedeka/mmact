...
def decode_video_frames(
    ...
    if backend == 'pyav': # NOTE: custom edit
        def decode_video_frames_pyav(
            ...
            with av.open(str(video_path)) as container:
                ...
                for ts in sorted(timestamps):
                    container.seek(int(ts * av.time_base ** -1), any_frame=False, backward=True, stream=stream)
                    stream.codec_context.skip_frame = "DEFAULT"
                    for frame in container.decode(stream):
                        frame_ts = float(frame.pts * stream.time_base)
                        if abs(frame_ts - ts) <= tolerance_s:
                            ...
                            break
                        if frame_ts > ts + tolerance_s:
                            break
                    stream.codec_context.skip_frame = "NONKEY"
            if not frames:
                raise ValueError(f"No frames found for timestamps {timestamps} in {video_path}")
