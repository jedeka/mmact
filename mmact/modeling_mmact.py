...
from lerobot.policies.mmact.configuration_mmact import mmACTConfig
...
#######
def check_content(x, prefix=''):
    for k,v in x.items(): 
        print(prefix, k, end=' ')
        print(v.shape) if isinstance(v, (torch.Tensor, np.ndarray)) else print(v)
    print(prefix,'\n', ':::'*20, '\n')


def get_processors(cfg, policy, dataset, device=torch.device('cpu')): 
...
def prep_vlm(dataset=None):
    ...
    # dataset_cfg = pkl_load('act_cfg_dataset.pkl')
    act_cfg = pkl_load('act_cfg.pkl')
    if dataset is None: 
        dataset = make_dataset(act_cfg)
    svla_cfg = TrainPipelineConfig(dataset)
    svla_cfg.policy = SmolVLAConfig() 
    svla = make_policy(
        cfg=svla_cfg.policy, 
    ...
    return {
        'preprocessor': svla_preprocessor,
        'postprocessor': svla_postprocessor, 
        'policy': svla,
        'cfg': svla_cfg.policy, 
        'cfg_dataset': act_cfg.dataset, 
    }
...
class mmACTPolicy(PreTrainedPolicy):
    ...
    print('*'*30); print('Using mmACT'); print('*'*30)
    # print(svla_dict)
...
class ACTEncoder(nn.Module):
    ...
    def __init__(self, config: mmACTConfig, is_vae_encoder: bool = False):
        super().__init__()
